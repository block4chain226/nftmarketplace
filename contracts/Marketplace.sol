...
//    mapping(address => uint256) private listersEarned;
    ...
    modifier getFee(uint256 price) {
        require(msg.value >= PROTOCOL_FEE + price, "you should pay fee");
        _;
    }
    ...
    function isDataValid(uint256 price, address seller, address token, uint256 tokenId, bytes memory signature) public view returns (bool) {
        bytes32 msgHash = keccak256(abi.encodePacked(msg.sender, price, seller, token, tokenId));
        require(
    ...
    function buy(uint256 price, address seller, address token, uint256 tokenId, bytes memory signature) external payable getFee(price) {
        require(msg.value > price, "not enough money");
        require(isDataValid(price, seller, token, tokenId, signature), "not right hash");
        IERC721(token).transferFrom(seller, msg.sender, tokenId);
        (bool success,) = msg.sender.call{value: price}("");
        require(success, "payment doesn't pass");
//        (bool success1,) = owner().call{value: PROTOCOL_FEE}("");
//        require(success1, "fee payment doesn't pass");
    }
