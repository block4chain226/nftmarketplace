...
const signTransaction = async (buyer, price, seller, contract, tokenId, owner) => {
...
describe("Deploy", function () {
    ...
    async function deploy() {
        const [owner, nftOwner, user1, user2, ...users] = await ethers.getSigners();
        const Token = await ethers.getContractFactory("ApeNft", nftOwner);
        const tokenContract = await Token.deploy();
        await tokenContract.waitForDeployment();

        for (let i = 0; i < 5; i++) {
            await tokenContract.mint(user1.address);
        }

        const Market = await ethers.getContractFactory("NftMarketplace", owner);
    ...
    describe("Market", () => {
        ...
        describe("recover", () => {
            it("recover function should return true", async () => {
                const {tokenContract, marketContract, nftOwner, owner, user1, user2} = await loadFixture(deploy);
                const hash = await signTransaction(user1, "300000", "0xBDf761788135C7d7Aa76E6671f63462A07C53E2C",
                    "0xb90bb21a90c2ab4b6857587713eabb43fd8de904", "2", owner);
                const priceFee = ethers.parseUnits("1000", "wei") + ethers.parseUnits("300000", "wei");
                const res = await marketContract.connect(user1).buy("300000", "0xBDf761788135C7d7Aa76E6671f63462A07C53E2C",
                    "0xb90bb21a90c2ab4b6857587713eabb43fd8de904", "2", hash, {value: priceFee});
            })
        })
    })
